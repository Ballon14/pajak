...
import OnlinePresenceIndicator, { AdminStatusController } from './OnlinePresenceIndicator'
...
export default function RealTimeChatSidebar() {
    ...
    useEffect(() => {
        if (isOpen && messages.length > 0) {
            ...
            unreadMessages.forEach(msg => markAsRead(msg.id, chatRoomId))
        }
    }, [isOpen, messages, session?.user?.id])
    ...
    const handleSendMessage = (e) => {
        ...
        sendMessage(message, null, chatRoomId)
        setMessage('')
        stopTyping(chatRoomId)
    }
        ...
        <>
            ...
            >
                <div className="relative">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:scale-110 transition-transform duration-300">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-slate-900 ${
                                adminStatus.status === 'online' ? 'bg-green-500' : 
                                adminStatus.status === 'busy' ? 'bg-yellow-500' : 'bg-gray-400'
                            }`}>
                                <div className={`absolute inset-0.5 rounded-full ${
                                    adminStatus.status === 'online' ? 'bg-green-400 animate-pulse' : ''
                                }`}></div>
                            </div>
                        </div>
                        <div>
                            <div className="font-bold text-xl tracking-tight">Support Chat</div>
                            <div className="text-sm text-slate-300">
                                {isConnected ? (
                                    adminStatus.status === 'online' ? '🟢 Admin Tersedia' : 
                                    adminStatus.status === 'busy' ? '🟡 Admin Sibuk' : '⚫ Admin Offline'
                                ) : '🔄 Menghubungkan...'}
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-4 h-full">
                                {Object.keys(groupedMessages).length === 0 && (
                                    <div className="text-center text-slate-500 py-16">
                                        ...
                                        <div className="font-semibold text-lg mb-2">Belum ada percakapan</div>
                                        <div className="text-sm text-slate-400">Mulai chat dengan admin sekarang</div>
                                    </div>
                                )}
                                                        ...
                                                        <div className={`max-w-[80%] ${isOwn ? 'order-2' : 'order-1'}`}>
                                                            {!isOwn && (
                                                                <div className="text-xs text-slate-500 mb-2 px-2 flex items-center gap-2">
                                                                    ...
                                                                    {msg.senderRole === 'admin' && (
                                                                        <span className="bg-blue-500 text-white px-2 py-0.5 rounded-full text-xs font-medium">Admin</span>
                                                                    )}
                                                                </div>
                                                            )}
                                                            <div className={`px-5 py-3 rounded-2xl shadow-sm ${
                                                                ...
                                                                <div className={`text-xs mt-2 flex items-center gap-1 ${
                                                                    isOwn ? 'text-blue-100 justify-end' : 'text-slate-500'
                                                                }`}>
                                                                    <span>{formatTime(msg.timestamp)}</span>
                                                                    {isOwn && (
                                                                        <span className={`ml-1 ${
                                                                            msg.status === 'read' ? 'text-green-300' : 
                                                                            msg.status === 'delivered' ? 'text-blue-300' : 'text-slate-300'
                                                                        }`}>
