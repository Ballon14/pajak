...
export async function POST(req) {
    try {
        ...
        const { message, from, email, userId, broadcast, isAdmin = false, targetUserId = null } = body
...
export async function GET(req) {
    try {
        ...
        if (isAdmin) {
            ...
            // User view: get their conversation messages
            let query = {}
            if (userId) {
...
export async function DELETE(req) {
    try {
        ...
        console.log("DELETE request received with:", { userId, email, conversationId })
        ...
        if (conversationId) {
            ...
            await db.collection("chats").deleteMany({ conversationId: new ObjectId(conversationId) })
        } else {
            // Validasi input
            if (!email && !userId) {
                return NextResponse.json(
                    { success: false, error: "Email, userId, atau conversationId diperlukan" },
                    { status: 400 }
                )
            }
